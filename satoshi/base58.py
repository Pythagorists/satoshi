...
def b58c_encode(payload, version_byte):
    ...
    checksum = sha256(sha256(unhexlify(extended_payload.encode('utf-8'))).digest()).digest()[:4]
    return b58_encode(
...
def b58c_decode(coded_string, application_byte=None):
    ...
    checksum = val[-8:]
    if checksum != sha256(sha256(unhexlify(extended_payload.encode('utf-8'))).digest()).hexdigest()[:8]:
